...
impl IndicatorProgress {
    ...
    fn calculate_trend(history: &[f64]) -> TrendDirection {
        ...
        let first_half_avg: f64 = recent[..recent.len() / 2].iter().sum::<f64>() / (recent.len() / 2) as f64;
        let second_half_avg: f64 = recent[recent.len() / 2..].iter().sum::<f64>() / (recent.len() - recent.len() / 2) as f64;
