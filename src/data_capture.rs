...
impl DataStorage {
    ...
    pub fn load_day_metadata(&self, date: NaiveDate) -> Result<Option<DayMetadata>> {
        ...
        let meta: DayMetadata = serde_json::from_str(&content)
            .context("Failed to parse day metadata")?;
    ...
    pub fn load_objectives(&self) -> Result<Vec<Objective>> {
        ...
        let container: ObjectivesContainer = serde_json::from_str(&content)
            .context("Failed to parse objectives")?;
    ...
    pub fn create_day_metadata_from_goals(&self, goals: &DailyGoals) -> DayMetadata {
        ...
        for (outcome_idx, outcome) in [&goals.work, &goals.health, &goals.family].iter().enumerate() {
