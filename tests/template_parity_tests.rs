...
use focusfive::models::{
    Action, ActionOrigin, ActionStatus, ActionTemplates, Config, DailyGoals, Outcome, OutcomeType,
};
...
mod integration_tests {
    ...
    fn test_morning_phase_template_application_integration() {
        ...
        assert_eq!(
            work_outcome.actions[2].text,
            "Prepare materials for first task"
        );
    ...
    fn test_template_application_with_action_limit_reached() {
        ...
        let original_actions: Vec<String> =
            outcome.actions.iter().map(|a| a.text.clone()).collect();
