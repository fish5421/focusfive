...
fn test_metadata_reconciliation_uses_action_ids() -> Result<()> {
    ...
    assert_eq!(meta.work[0].status, focusfive::models::ActionMetaStatus::Done);
    assert_eq!(meta.work[1].status, focusfive::models::ActionMetaStatus::Planned);
    assert_eq!(meta.work[2].status, focusfive::models::ActionMetaStatus::Planned);
...
fn test_metadata_reconciliation_on_action_add() -> Result<()> {
    ...
    // Add a third action
    goals.work.actions.push(Action::from_markdown("Task 3".to_string(), true));
    ...
    assert_eq!(meta.work[2].status, focusfive::models::ActionMetaStatus::Done);
...
fn test_metadata_status_sync_with_completion() -> Result<()> {
    ...
    let mut meta = DayMeta::from_goals(&goals);
    assert_eq!(meta.work[0].status, focusfive::models::ActionMetaStatus::Planned);
...
fn test_metadata_preserves_in_progress_status() -> Result<()> {
    ...
    meta.reconcile_with_goals(&goals);
    assert_eq!(meta.work[0].status, focusfive::models::ActionMetaStatus::InProgress);
...
fn test_metadata_preserves_blocked_status() -> Result<()> {
    ...
    meta.reconcile_with_goals(&goals);
    assert_eq!(meta.work[0].status, focusfive::models::ActionMetaStatus::Blocked);
