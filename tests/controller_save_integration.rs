...
fn test_controller_saves_all_data_types() -> Result<()> {
    ...
    app.templates.add_template("Test Template".to_string(), vec![
        "Action 1".to_string(),
        "Action 2".to_string(),
        "Action 3".to_string(),
    ]);
    ...
    let meta_file = temp_dir.path().join("data").join("meta").join("2025-08-29.meta.json");
...
fn test_atomic_write_prevents_corruption() -> Result<()> {
    ...
    println!("Concurrent saves - Success: {}, Errors: {}", successes, errors);
    ...
    assert_eq!(loaded.objectives.len(), 1, "Should have exactly one objective");
    assert!(loaded.objectives[0].id.starts_with("thread-"), "Should be from one of the threads");
